...
class SocialLinks {
  ...
  factory SocialLinks.fromJson(dynamic json) {
    ...
    if (json is Map<String, dynamic>) {
      ...
      debugPrint('SocialLinks: Unexpected data type ${json.runtimeType}, returning empty SocialLinks');
      return SocialLinks();
    }

    if (socialLinksMap == null) {
      debugPrint('SocialLinks: Could not extract map, returning empty SocialLinks');
      return SocialLinks();
    }
    ...
    debugPrint('SocialLinks created: facebook=${socialLinks.facebook}, instagram=${socialLinks.instagram}, twitter=${socialLinks.twitter}, youtube=${socialLinks.youtube}');
    return socialLinks;
  }
...
class Artist {
  ...
  factory Artist.fromJson(Map<String, dynamic> json) {
    ...
    bool hasNoSocialLinks = socialLinks.facebook == null && socialLinks.instagram == null &&
                           socialLinks.twitter == null && socialLinks.youtube == null;

    if (bio != null && bio.contains('http') && hasNoSocialLinks) {
      debugPrint('Found URL in bio field, attempting to parse as social link: $bio');

      // Try to determine which social platform this is
      if (bio.toLowerCase().contains('facebook') || bio.toLowerCase().contains('fb.com')) {
      ...
      } else if (bio.toLowerCase().contains('instagram') || bio.toLowerCase().contains('insta')) {
        socialLinks = SocialLinks(instagram: bio);
        bio = null;
        debugPrint('Moved Instagram URL from bio to socialLinks');
      } else if (bio.toLowerCase().contains('twitter') || bio.toLowerCase().contains('x.com')) {
        socialLinks = SocialLinks(twitter: bio);
        bio = null;
        debugPrint('Moved Twitter URL from bio to socialLinks');
      } else if (bio.toLowerCase().contains('youtube') || bio.toLowerCase().contains('youtu.be')) {
